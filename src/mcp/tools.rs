...
pub fn tool_list() -> Value {
    ...
    Value::Arr(vec![
        ...
        tool("reindex", "Rebuild the binary inverted index from all topic files.",
            &[], &[]),
        tool("session", "Show session defaults. Use action param: set_phase (research/build/verify/debug), add_focus (track a topic), note (queue text for batch storage on stop). Checkpoint/resume: use action='checkpoint' to save where you left off (survives across sessions), action='resume' to load it back, action='clear_checkpoint' to clear.",
            &[],
            &[("action", "string", "Operation: show (default), set_phase, add_focus, note, checkpoint, resume, clear_checkpoint"),

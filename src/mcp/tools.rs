...
pub fn tool_list() -> Value {
    ...
    Value::Arr(vec![
        ...
        tool("callgraph", "Trace callers and callees of a function across a codebase. Shows call chains as an indented tree with recursion up to depth levels.",
            &["pattern", "path"],
            &[("pattern", "string", "Function name to trace"),
              ("path", "string", "Codebase directory to search"),
              ("glob", "string", "File filter suffix (default: *.rs)"),
              ("depth", "string", "Recursion depth for caller chains (default: 2, max: 3)"),
              ("direction", "string", "callers|callees|both (default: both)"),
              ("store_topic", "string", "If set, store results under this amaranthine topic"),
              ("tags", "string", "Tags for stored entry (default: structural,callgraph)")]),
        tool("codepath", "Search a codebase directory for a pattern, categorize access sites by type (clone, comparison, map_key, format_arg, method_call, field_access). Returns structured coupling profile. Optionally store results for future retrieval.",
            &["pattern", "path"],

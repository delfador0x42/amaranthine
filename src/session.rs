...
impl Checkpoint {
    ...
    pub fn format_resume(&self) -> String {
        ...
        if hours > 0 {
            out.push_str(&format!("_saved {}h {}m ago_\n", hours, mins));
        } else {
            out.push_str(&format!("_saved {}m ago_\n", mins));
        }
    ...
    fn to_json(&self) -> String {
        ...
        b.push_str("\",\n  \"done\": ");
        json_str_array(&mut b, &self.done);
        b.push_str(",\n  \"next\": ");
        json_str_array(&mut b, &self.next);
        b.push_str(",\n  \"hypotheses\": ");
        json_str_array(&mut b, &self.hypotheses);
        b.push_str(",\n  \"blocked\": \"");
        crate::json::escape_into(&self.blocked, &mut b);
        b.push_str("\",\n  \"files\": ");
        json_str_array(&mut b, &self.files);
        b.push_str(",\n  \"timestamp\": ");
        push_u64(&mut b, self.timestamp);
        b.push_str("\n}\n");
        b
    }
